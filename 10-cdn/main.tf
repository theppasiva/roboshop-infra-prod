resource "aws_cloudfront_distribution" "shivarampractise" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.shivarampractise.domain_name
        zone_id = aws_cloudfront_distribution.shivarampractise.hosted_zone_id
      }
    }
  ]
}